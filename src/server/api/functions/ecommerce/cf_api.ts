...
export async function getCategories() {
  ...
  return (await parseApiResponse(response, CategoryResult)).product_categories;
}

export async function getDepartments() {
  const response = await makeApiRequest("get_product_departments");
  return (await parseApiResponse(response, DepartmentResult))
    .product_departments;
}

export async function getManufacturers() {
  const response = await makeApiRequest("get_product_manufacturers");
  return (await parseApiResponse(response, ManufacturerResult))
    .product_manufacturers;
}

export async function getTags() {
  const response = await makeApiRequest("get_product_tags");
  return (await parseApiResponse(response, TagResult)).product_tags;
}

export async function getLocations() {
  const response = await makeApiRequest("get_locations");
  return (await parseApiResponse(response, LocationResult)).locations;
}
